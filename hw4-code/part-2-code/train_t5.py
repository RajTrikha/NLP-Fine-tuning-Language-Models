...
def find_data_folder():
    '''Find the data folder by checking multiple possible locations'''
    possible_paths = [
        'data',  # Same directory
    ...
    for path in possible_paths:
        if os.path.exists(os.path.join(path, 'dev.sql')):
            return path

    # If not found, return 'data' as default (will fail with clear error)
    return 'data'
...
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    total_loss = 0
    total_tokens = 0
    # Use ignore_index=-100 (PyTorch standard for ignoring padding in loss)
    ...
    for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(train_loader):
        ...
        # Flatten for CrossEntropyLoss (standard PyTorch pattern)
        ...
        with torch.no_grad():
            # Count non-ignored tokens (where target != -100)
